"""
VibeVoice Inference with Caption Generation

This script demonstrates how to use VibeVoice with automatic caption generation
for accessibility and content understanding.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="VibeVoice Inference with Caption Generation")
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Whisper model size for caption generation",
    )
    parser.add_argument(
        "--caption_formats",
        type=str,
...
def main():
    ...
    if args.generate_captions:
        ...
        # Create speaker mapping for captions
        speaker_mapping = {i: actual_speakers[i] for i in range(len(actual_speakers))}
        ...
        result = processor.save_audio_with_captions(
            ...
            caption_formats=args.caption_formats
        )
        
        print(f"Saved audio to: {result['audio_paths']}")
        
        if result.get('caption_files'):
            print(f"📝 Generated caption files:")
            for format_name, file_path in result['caption_files'].items():
