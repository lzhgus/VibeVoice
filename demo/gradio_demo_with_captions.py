"""
VibeVoice Gradio Demo with Caption Support

Enhanced demo interface that includes automatic caption generation for generated audio,
providing accessibility features and content understanding capabilities.
"""
...
class VibeVoiceDemoWithCaptions:
    def __init__(self, model_path: str, device: str = "cuda", inference_steps: int = 5, 
                 caption_model: str = "base", caption_language: str = None):
        """Initialize the VibeVoice demo with caption support."""
    ...
    def load_model(self):
        """Load the VibeVoice model and processor with caption support."""
        ...
        # Load processor with caption support
        self.processor = VibeVoiceProcessorWithCaptions.from_pretrained(
            self.model_path,
            caption_model=self.caption_model,
            caption_device=self.device,
    ...
    def generate_podcast_with_captions(self, 
                                     ...
                                     caption_formats: List[str] = None) -> Iterator[tuple]:
        """
        Generate podcast with automatic caption generation.
        
        Args:
            num_speakers: Number of speakers
            script: Podcast script
            speaker_1-4: Speaker voice selections
        ...
        Yields:
            Tuple of (streaming_audio, complete_audio, log, streaming_visible)
        """
        try:
            ...
            if generate_captions:
                log += f"📝 Captions: Enabled (formats: {', '.join(caption_formats or ['srt', 'vtt'])})"
            else:
                log += f"📝 Captions: Disabled"
            ...
            if all_audio_chunks:
                ...
                if generate_captions:
                    try:
                        # Save audio temporarily for caption generation
                        temp_audio_path = f"temp_audio_{int(time.time())}.wav"
                        sf.write(temp_audio_path, complete_audio, sample_rate)
                        
                        # Create speaker mapping
                        speaker_mapping = {i: selected_speakers[i] for i in range(len(selected_speakers))}
                        
                        # Generate captions
                        caption_result = self.processor.save_audio_with_captions(
                            ...
                            caption_formats=caption_formats or ['srt', 'vtt', 'json', 'transcript']
                        )
                        
                        if caption_result.get('caption_files'):
                            caption_info = f"\n📝 Captions generated: {len(caption_result['caption_files'])} files"
                            for format_name, file_path in caption_result['caption_files'].items():
                                caption_info += f"\n   - {format_name}: {file_path}"
                        else:
                            caption_info = "\n📝 Caption generation attempted but no files created"
                        ...
                        caption_info = f"\n📝 Caption generation failed: {str(e)}"
...
def create_demo_interface_with_captions(demo_instance: VibeVoiceDemoWithCaptions):
    """Create the enhanced Gradio interface with caption support."""
    ...
    with gr.Blocks(
        title="VibeVoice - AI Podcast Generator with Captions",
        css=custom_css,
        ...
        <div class="main-header">
            <h1>🎙️ Vibe Podcasting with Captions</h1>
            <p>Generating Long-form Multi-speaker AI Podcast with VibeVoice + Automatic Caption Generation</p>
        </div>
        """)
            ...
            with gr.Column(scale=1, elem_classes="settings-card"):
                ...
                with gr.Row():
                    enable_captions = gr.Checkbox(
                        ...
                        value=True,
                        info="Generate automatic captions for accessibility"
                    )
                    ...
                    caption_model = gr.Dropdown(
                        ...
                        value="base",
                        label="Caption Model",
                        info="Whisper model size (base recommended)"
                    )
                
                caption_formats = gr.CheckboxGroup(
                    choices=["srt", "vtt", "json", "transcript"],
                    value=["srt", "vtt", "transcript"],
                    label="Caption Formats",
                    info="Select caption output formats"
                )
...
Or paste text directly and it will auto-assign speakers.""",
                ...
                with gr.Row():
                    ...
                    generate_btn = gr.Button(
                        "🚀 Generate Podcast with Captions",
                        size="lg",
                        variant="primary",
                ...
                *📝 **Captions**: Automatic caption files will be generated if enabled*
                """)
        ...
        # Main generation function with streaming and captions
        def generate_podcast_wrapper(num_speakers, script, *speakers_and_params):
            """Wrapper function to handle the streaming generation call with captions."""
            try:
                # Extract speakers and parameters
                speakers = speakers_and_params[:4]  # First 4 are speaker selections
                enable_captions = speakers_and_params[4]   # Caption enable flag
                caption_model = speakers_and_params[5]     # Caption model
                caption_formats = speakers_and_params[6]   # Caption formats
                cfg_scale = speakers_and_params[7]         # CFG scale
                ...
                for streaming_audio, complete_audio, log, streaming_visible in demo_instance.generate_podcast_with_captions(
                    ...
                    cfg_scale=cfg_scale,
                    generate_captions=enable_captions,
        ...
        generate_btn.click(
            ...
            inputs=[num_speakers, script_input] + speaker_selections + [enable_captions, caption_model, caption_formats, cfg_scale],
        ...
        - Click **🚀 Generate Podcast with Captions** to start audio generation
        ...
        - **Captions** are automatically generated in multiple formats for accessibility
...
def parse_args():
    parser = argparse.ArgumentParser(description="VibeVoice Gradio Demo with Captions")
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="base",
        help="Whisper model size for caption generation: tiny | base | small | medium | large",
    )
    parser.add_argument(
        "--caption_language",
        type=str,
        default=None,
        help="Language for caption generation (e.g., 'en', 'zh'). If None, auto-detects.",
    )
    parser.add_argument(
        "--share",
        action="store_true",
...
def main():
    """Main function to run the demo with caption support."""
    args = parse_args()
    
    set_seed(42)  # Set a fixed seed for reproducibility

    print("🎙️ Initializing VibeVoice Demo with Caption Support...")
    
    # Initialize demo instance
    demo_instance = VibeVoiceDemoWithCaptions(
    ...
    # Create interface
    interface = create_demo_interface_with_captions(demo_instance)
    
    print(f"🚀 Launching demo on port {args.port}")
    print(f"📁 Model path: {args.model_path}")
    print(f"🎭 Available voices: {len(demo_instance.available_voices)}")
    print(f"🔴 Streaming mode: ENABLED")
    print(f"📝 Caption generation: ENABLED (model: {args.caption_model})")
